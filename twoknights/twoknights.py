...
def get_next_states(s1,s2,n,line):
	states=[]
	for ns1 in valid_neighbours(s1):
		...
		if not c:
			states.append((ns1,s2,n))
		elif line[n]==c:
			states.append((ns1,s2,n+1))
	for ns2 in valid_neighbours(s2):
		...
		if not c:
			states.append((s1,ns2,n))
		elif line[n]==c:
			states.append((s1,ns2,n+1))
	return states
...
def cantype(line,s1,s2):
	...
	while q:
		...
		if n==len(line):
			return True
		nbrs=get_next_states(s1,s2,n,line)
		for ns1,ns2,nn in nbrs:
			nhs=hash_state_with_size(ns1,ns2,nn)
